...
class SelectableHospitallItem(val hospital : HospitalResult) :
    ...
    override fun bindView(binding: ItemGeneralSelectableBinding, payloads: List<Any>) {
        ...
        binding.itemGeneralCheckbox.text = hospital.name
    }


    class CheckBoxClickEvent(val invoke : (HospitalResult) -> Unit) : ClickEventHook<SelectableHospitallItem>() {
        ...
        override fun onClick(
            v: View,
            position: Int,
            fastAdapter: FastAdapter<SelectableHospitallItem>,
            item: SelectableHospitallItem
        ) {
            val selectExtension: SelectExtension<SelectableHospitallItem> = fastAdapter.requireExtension()
            selectExtension.toggleSelection(position)


            invoke(item.hospital)
        }
